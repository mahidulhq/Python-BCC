...
# Important notes:
# - This app sends a plain text email using Gmail SMTP over TLS.
# - Fill in sender email, app password, receiver email, subject, and message.
# - Click "Send Email" to run send_email().
# - If any field is empty, a warning is shown.
# - On success, a confirmation popup appears; on failure, the error is shown.
# - For Gmail, use an App Password (regular account passwords are often blocked).
# - To create an App Password: enable 2-Step Verification, open App Passwords,
#   create one for Mail, and use that password in this app.
# - Never hardcode or share real credentials in source code.